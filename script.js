...
function createHeart() {
    ...
    he.classList.add('he');
    he.style.left = `${Math.random() * window.innerWidth}px`;
    he.style.animationDuration = `${Math.random() * 5 + 10}s`;
    document.getElementById('he-container').appendChild(he);

    setTimeout(() => {
        he.remove();
    }, 15000);

    const hea = document.createElement('div');
    hea.classList.add('hea');
    hea.style.left = `${Math.random() * window.innerWidth}px`;
    hea.style.animationDuration = `${Math.random() * 5 + 10}s`;
    document.getElementById('hea-container').appendChild(hea);
